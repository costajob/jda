...
801613068  ,20162  ,"Verona                        ",0        ,14       ,0        ,100  ,"HANDBAGS                 ",1       ,"141576F4FXG    ",1000  ,"0099","1000/ /JACKIE SM HOBO ORIG GG ",294.00         ,"Y",70707   ,0      ,1} }
  ...
  describe "#filter!" do
    ...
    it "must filter by skus" do
      ...
      parser.files.each do |file|
        parser.cache[file].value.size.must_equal 3
      end
    end
    ...
    it "must filter by store codes" do
      ...
      parser.files.each do |file|
        parser.cache[file].value.size.must_equal 4
      end
    end
    ...
    it "must filter by markdown flag" do
      ...
      parser.files.each do |file|
        parser.cache[file].value.size.must_equal 5
      end
    end
    ...
    it "must combine filters" do
      ...
      parser.files.each do |file|
        parser.cache[file].value.size.must_equal 1
      end
    end
  end
